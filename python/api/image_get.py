...
class ImageGet(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        # Handle /a0/ paths by converting them to actual container paths
        if path.startswith("/a0/"):
            path = path.replace("/a0/", "/root/projects/pareng-boyong/")
        
        # For full VPS access, we allow any valid path (not just base directory)
        # This enables Pareng Boyong to access the entire VPS filesystem
        if not os.path.exists(files.get_abs_path(path) if not os.path.isabs(path) else path):
            # Try with fix_dev_path for backward compatibility
            if runtime.is_development():
                path = files.fix_dev_path(path)
        
        # Remove the restrictive base directory check to allow full VPS access
        # Pareng Boyong should be able to access any file on the VPS

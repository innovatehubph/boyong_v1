# kokoro_tts.py - Fallback TTS system for Pareng Boyong
...
async def _preload():
    ...
    try:
        ...
        if not _pipeline:
            PrintStyle.standard("Loading Kokoro TTS model as fallback...")
            from kokoro import KPipeline
            _pipeline = KPipeline(lang_code="a")
            PrintStyle.standard("✅ Kokoro TTS fallback ready")
    finally:
        is_updating_model = False
...
async def synthesize_sentences(sentences: list[str]):
    ...
    try:
        ...
        PrintStyle.error(f"Kokoro TTS fallback failed: {e}")
        # Return empty string to trigger browser TTS fallback
        return ""

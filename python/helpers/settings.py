...
class Settings(TypedDict):
    ...
    stt_provider: str
    stt_elevenlabs_enable: bool
    stt_elevenlabs_model: str

    tts_kokoro: bool
    tts_elevenlabs: bool
    tts_toucan: bool
    tts_browser_fallback: bool
    tts_provider_priority: str
    tts_elevenlabs_voice_id: str
    tts_elevenlabs_stability: float
    tts_elevenlabs_similarity: float
    tts_elevenlabs_style: float
    tts_elevenlabs_speaker_boost: bool
    tts_speech_to_speech_enable: bool
    tts_speech_to_speech_voice_id: str
    tts_speech_to_speech_stability: float
    tts_speech_to_speech_model: str
    tts_speech_to_speech_remove_noise: bool
    tts_speech_to_speech_streaming: bool
    tts_speech_to_speech_output_format: str
    stt_elevenlabs_diarization: bool
    stt_elevenlabs_timestamps: str
...
def convert_out(settings: Settings) -> SettingsOutput:
    ...
    stt_fields.append(
        {
            ...
            "description": "Choose the STT provider. ElevenLabs offers higher accuracy for supported languages.",
            "type": "select",
            "value": settings["stt_provider"],
            "options": [
                {"value": "whisper", "label": "Whisper (Local, Free)"},
                {"value": "elevenlabs", "label": "ElevenLabs (Cloud, Premium)"},
    ...
    stt_fields.append(
        {
            "id": "stt_elevenlabs_enable",
            "title": "Enable ElevenLabs STT",
            "description": "Enable premium ElevenLabs Speech-to-Text for higher accuracy transcription.",
            "type": "switch",
            "value": settings["stt_elevenlabs_enable"],
        }
    )

    stt_fields.append(
        {
            "id": "stt_elevenlabs_model",
            "title": "ElevenLabs STT Model",
            "description": "Select ElevenLabs Speech-to-Text model for transcription.",
            "type": "select",
            "value": settings["stt_elevenlabs_model"],
            "options": [
                {"value": "scribe_v1", "label": "Scribe V1 (Production Ready)"},
                {"value": "scribe_v1_experimental", "label": "Scribe V1 Experimental (Latest Features)"},
            ],
        }
    )
    ...
    stt_fields.append(
        {
            "id": "stt_elevenlabs_diarization",
            "title": "Enable Speaker Diarization",
            "description": "Identify and separate different speakers in the audio (who said what).",
            "type": "switch",
            "value": settings["stt_elevenlabs_diarization"],
        }
    )
    
    stt_fields.append(
        {
            "id": "stt_elevenlabs_timestamps",
            "title": "Timestamp Granularity",
            "description": "Level of detail for word timestamps in transcription.",
            "type": "select",
            "value": settings["stt_elevenlabs_timestamps"],
            "options": [
                {"value": "word", "label": "Word Level (Recommended)"},
                {"value": "character", "label": "Character Level (Detailed)"},
                {"value": "none", "label": "No Timestamps"},
            ],
        }
    )

    # Traditional Whisper Settings (for compatibility)
    stt_fields.append(
        {
            "id": "stt_model_size",
            "title": "Whisper model size",
            "description": "Select the Whisper model size (used when Whisper provider is selected)",
            "type": "select",
    ...
    tts_fields.append(
        {
            "id": "tts_provider_priority",
            "title": "TTS Provider Priority",
            "description": "Choose the primary TTS provider. 'Auto' uses intelligent selection based on content language.",
            "type": "select",
            "value": settings["tts_provider_priority"],
            "options": [
                {"value": "auto", "label": "Auto (Intelligent Selection)"},
                {"value": "elevenlabs", "label": "ElevenLabs (Premium Quality)"},
                {"value": "toucan", "label": "ToucanTTS (Filipino Optimized)"},
                {"value": "kokoro", "label": "Kokoro TTS (Fallback)"},
                {"value": "browser", "label": "Browser TTS (Basic)"},
            ],
        }
    )
    ...
    tts_fields.append(
        {
            "id": "tts_elevenlabs",
            "title": "Enable ElevenLabs TTS",
            "description": "Enable premium quality ElevenLabs TTS with custom voice (Kuya Archer). Requires API key.",
            "type": "switch",
            "value": settings["tts_elevenlabs"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_elevenlabs_voice_id",
            "title": "ElevenLabs Voice ID",
            "description": "Custom voice ID for ElevenLabs. Default is Kuya Archer (ZmDKIF1W70PJVAMJee2h).",
            "type": "text",
            "value": settings["tts_elevenlabs_voice_id"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_elevenlabs_stability",
            "title": "Voice Stability",
            "description": "Higher values make voice more stable but less expressive (0.0 - 1.0)",
            "type": "range",
            "min": 0.0,
            "max": 1.0,
            "step": 0.05,
            "value": settings["tts_elevenlabs_stability"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_elevenlabs_similarity",
            "title": "Voice Similarity Boost",
            "description": "How closely the AI should follow the original voice (0.0 - 1.0)",
            "type": "range",
            "min": 0.0,
            "max": 1.0,
            "step": 0.05,
            "value": settings["tts_elevenlabs_similarity"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_elevenlabs_style",
            "title": "Voice Style Enhancement",
            "description": "Adds more character and expressiveness to the voice (0.0 - 1.0)",
            "type": "range",
            "min": 0.0,
            "max": 1.0,
            "step": 0.05,
            "value": settings["tts_elevenlabs_style"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_elevenlabs_speaker_boost",
            "title": "Speaker Boost",
            "description": "Enhance voice clarity and reduce background noise",
            "type": "switch",
            "value": settings["tts_elevenlabs_speaker_boost"],
        }
    )
    ...
    tts_fields.append(
        {
            "id": "tts_toucan",
            "title": "Enable ToucanTTS",
            "description": "Enable ToucanTTS for Filipino/Tagalog language optimization with natural pronunciation.",
            "type": "switch",
            "value": settings["tts_toucan"],
        }
    )
    ...
    tts_fields.append(
        {
            ...
            "description": "Enable Kokoro TTS as a fallback option when other TTS providers are unavailable.",
            "type": "switch",
    ...
    tts_fields.append(
        {
            "id": "tts_browser_fallback",
            "title": "Enable Browser TTS Fallback",
            "description": "Allow fallback to browser-based text-to-speech when all server-side options fail.",
            "type": "switch",
            "value": settings["tts_browser_fallback"],
        }
    )
    ...
    tts_fields.append(
        {
            "id": "tts_speech_to_speech_enable",
            "title": "Enable Speech-to-Speech",
            "description": "Enable ElevenLabs Speech-to-Speech for voice conversion and accent modification.",
            "type": "switch",
            "value": settings["tts_speech_to_speech_enable"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_speech_to_speech_voice_id",
            "title": "Speech-to-Speech Target Voice",
            "description": "Voice ID to convert speech into. Uses same voice as TTS if empty.",
            "type": "text",
            "value": settings["tts_speech_to_speech_voice_id"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_speech_to_speech_stability",
            "title": "Speech-to-Speech Stability",
            "description": "Stability for speech-to-speech conversion (0.0 - 1.0)",
            "type": "range",
            "min": 0.0,
            "max": 1.0,
            "step": 0.05,
            "value": settings["tts_speech_to_speech_stability"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_speech_to_speech_model",
            "title": "Speech-to-Speech Model",
            "description": "AI model for speech-to-speech conversion.",
            "type": "select",
            "value": settings["tts_speech_to_speech_model"],
            "options": [
                {"value": "eleven_english_sts_v2", "label": "English STS V2 (Recommended)"},
                {"value": "eleven_multilingual_sts_v2", "label": "Multilingual STS V2"},
            ],
        }
    )
    ...
    tts_fields.append(
        {
            "id": "tts_speech_to_speech_remove_noise",
            "title": "Remove Background Noise",
            "description": "Clean background noise from source audio before conversion.",
            "type": "switch",
            "value": settings["tts_speech_to_speech_remove_noise"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_speech_to_speech_streaming",
            "title": "Enable Streaming Mode",
            "description": "Use streaming for faster speech-to-speech processing.",
            "type": "switch",
            "value": settings["tts_speech_to_speech_streaming"],
        }
    )
    
    tts_fields.append(
        {
            "id": "tts_speech_to_speech_output_format",
            "title": "Output Audio Format",
            "description": "Audio format for speech-to-speech output.",
            "type": "select",
            "value": settings["tts_speech_to_speech_output_format"],
            "options": [
                {"value": "mp3_44100_128", "label": "MP3 44.1kHz 128kbps (Recommended)"},
                {"value": "mp3_22050_32", "label": "MP3 22kHz 32kbps (Compact)"},
                {"value": "pcm_16000", "label": "PCM 16kHz (Raw)"},
                {"value": "pcm_22050", "label": "PCM 22kHz (Raw)"},
                {"value": "pcm_44100", "label": "PCM 44.1kHz (Raw)"},
            ],
        }
    )
...
def get_default_settings() -> Settings:
    ...
    return Settings(
        ...
        chat_model_ctx_length=120000, # Enhanced from 100k to 120k for better quality (most models support 128k+)
        chat_model_ctx_history=0.75,  # Optimized balance: more history than before but smart compression
        chat_model_vision=True,
        chat_model_rl_requests=0,
        chat_model_rl_input=0,
        chat_model_rl_output=0,
        util_model_provider=ModelProvider.OPENROUTER.name,
        util_model_name="openai/gpt-4o-mini",  # Better quality than nano, still cost-effective
        util_model_api_base="",
        ...
        stt_elevenlabs_diarization=False,
        stt_elevenlabs_timestamps="word",
        tts_kokoro=True,
        tts_elevenlabs=True,
        tts_toucan=True,
        tts_browser_fallback=True,
        tts_provider_priority="auto",
        tts_elevenlabs_voice_id="ZmDKIF1W70PJVAMJee2h",  # Kuya Archer voice
        ...
        tts_speech_to_speech_voice_id="21m00Tcm4TlvDq8ikWAM",  # Rachel voice (default)
        tts_speech_to_speech_stability=0.75,
        tts_speech_to_speech_model="eleven_english_sts_v2",
        tts_speech_to_speech_remove_noise=False,
        tts_speech_to_speech_streaming=True,
        tts_speech_to_speech_output_format="mp3_44100_128",
        mcp_servers='{\n    "mcpServers": {}\n}',

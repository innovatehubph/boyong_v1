...
# Import the path resolver for permanent /a0/ path fix
try:
    from python.helpers.path_resolver import resolve_path
except ImportError:
    # Fallback if path_resolver is not available
    def resolve_path(path):
        if path and str(path).startswith("/a0/"):
            return str(path).replace("/a0/", "/root/projects/pareng-boyong/")
        return path
...
def get_abs_path(*relative_paths):
    """Convert relative paths to absolute paths with permanent /a0/ path resolution.
    
    This function now uses the centralized path_resolver to ensure all /a0/ paths
    are permanently mapped to actual VPS paths, removing the imaginary boundary.
    """
    if not relative_paths:
        return get_base_dir()
    
    # Join all paths first
    if len(relative_paths) == 1:
        path = relative_paths[0]
    else:
        path = os.path.join(*relative_paths)
    
    # Use the path resolver to handle /a0/ paths
    resolved = resolve_path(path)
    
    # If it's not an absolute path after resolution, join with base directory
    if not os.path.isabs(resolved):
        resolved = os.path.join(get_base_dir(), resolved)
...
def fix_dev_path(path:str):
    "Enhanced path resolution for full VPS access (no longer restricted to /a0/)"
    from python.helpers.runtime import is_development
    
    # If already an absolute VPS path, use it directly
    if path.startswith("/") and not path.startswith("/a0/"):
        return path
    
    if is_development():
        # Convert old /a0/ paths to VPS absolute paths
        if path.startswith("/a0/"):
            # Map /a0/ to the actual container working directory
            path = path.replace("/a0/", "/root/projects/pareng-boyong/")
            return path

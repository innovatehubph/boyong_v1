"""
Image Generation Tool for Pareng Boyong
...
<image>{image_base64}</image>""",
    ...
    async def _generate_image(
        ...
        try:
            # Try new multimedia system first
            from pareng_boyong_multimedia import multimedia_image_generator
            
            # Convert style to category for new system
            category_map = {
            ...
            category = category_map.get(model_preference, "artwork")
            
            result = multimedia_image_generator(prompt, category, width, height)
            if result and result.get("status") == "success":
                # Extract base64 content for compatibility
            ...
            PrintStyle.warning("Image generation modules not available")
            return None
        except Exception as e:

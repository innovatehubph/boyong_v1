...
const model = {
  ...
  // TTS Settings - ElevenLabs enabled by default with valid API key
  tts_kokoro: true,   // Fallback TTS
  tts_elevenlabs: true,  // Primary high-quality TTS
  tts_toucan: true,   // Filipino TTS
  ...
  // speak wrapper - Pareng Boyong multi-TTS integration
  async _speak(text, waitForPrevious, terminator) {
    // Try Toucan TTS for Filipino content if enabled
    if (this.tts_toucan && this._isFilipinoText(text)) {
      try {
        await this.speakWithToucanTTS(text, waitForPrevious, terminator);
        return;
      } catch (error) {
        console.error("Toucan TTS failed, falling back to ElevenLabs:", error);
      }
    }
    
    // Use ElevenLabs TTS (Kuya Archer) if enabled, fallback to browser TTS
    if (this.tts_elevenlabs) {
      try {
        await this.speakWithElevenLabs(text, waitForPrevious, terminator);
        return;
      } catch (error) {
  ...
  // Helper function to detect Filipino text
  _isFilipinoText(text) {
    const filipinoIndicators = [
      'ang', 'ng', 'sa', 'na', 'si', 'ni', 'kay', 'para', 'kung', 'ako', 'ikaw', 'siya',
      'kami', 'tayo', 'kayo', 'sila', 'ko', 'mo', 'niya', 'namin', 'natin', 'ninyo', 'nila',
      'kumusta', 'salamat', 'paalam', 'mahal kita', 'magandang', 'umaga', 'gabi', 'hapon',
      'pareng', 'boyong', 'kuya', 'ate', 'tita', 'tito', 'lola', 'lolo',
      'ba', 'po', 'opo', 'hindi', 'oo', 'ito', 'yan', 'yun', 'dito', 'dyan', 'doon'
    ];
    
    const textLower = text.toLowerCase();
    const filipinoWordCount = filipinoIndicators.filter(word => textLower.includes(word)).length;
    const totalWords = text.split(' ').length;
    
    return filipinoWordCount > 0 && (filipinoWordCount / Math.max(totalWords, 1)) > 0.2;
  },
  ...
  // Show a quiet prompt to user to enable audio (less annoying)
  showAudioPermissionPrompt() {
    // Only show prompt once per session to avoid spam
    if (!this.audioPromptShown) {
      this.audioPromptShown = true;
      console.log("🎤 Pareng Boyong audio ready - interact with page to enable speech");
      
      // Optional: Show a subtle toast only if toast system exists
      if (window.toast) {
        window.toast("🎤 Audio ready - interact to enable Kuya Archer voice", "info", 3000);
      }
    }
  },
  ...
  // ElevenLabs TTS with Kuya Archer voice
  async speakWithElevenLabs(text, waitForPrevious = false, terminator = null) {
    try {
      // synthesize on the backend using ElevenLabs
      ...
      if (response.success) {
        if (response.audio) {
          // Play the synthesized audio from ElevenLabs
          await this.playAudio(response.audio);
        } else if (response.use_browser_tts) {
          // Server signaled to use browser TTS fallback
          console.log("Falling back to browser TTS as requested by server");
          await this.speakWithBrowser(text, false, terminator);
        } else {
          throw new Error("No audio data or fallback signal received");
        }
      } else {
        throw new Error(`ElevenLabs TTS error: ${response.error}`);
      }
    } catch (error) {
  ...
  // Toucan TTS for Filipino language
  async speakWithToucanTTS(text, waitForPrevious = false, terminator = null) {
    try {
      // synthesize on the backend using Toucan TTS
      const response = await sendJsonData("/synthesize", { text, language: "filipino" });
      ...
      if (response.success) {
        if (response.audio) {
          // Play the synthesized audio from Toucan TTS
          await this.playAudio(response.audio);
        } else if (response.use_browser_tts) {
          // Server signaled to use browser TTS fallback
          console.log("Falling back to browser TTS as requested by server");
        ...
        throw new Error(`Toucan TTS error: ${response.error}`);
      }
    } catch (error) {
      throw new Error(`Toucan TTS error: ${error.message}`);
    }
  },

...
const model = {
  ...
  getServerImgUrl(filename) {
    return `/image_get?path=/root/projects/pareng-boyong/tmp/uploads/${encodeURIComponent(filename)}`;
  },

  getServerFileUrl(filename) {
    return `/root/projects/pareng-boyong/tmp/uploads/${encodeURIComponent(filename)}`;
  },

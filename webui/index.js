...
async function poll() {
  ...
  try {
    ...
    if (document.body.getAttribute('x-data')) {
      const bodyData = Alpine.$data(document.body);
      if (bodyData && bodyData.isLoading) {
...
async function startPolling() {
  const shortInterval = 500;  // Increased from 25ms to 500ms
  const longInterval = 2000;  // Increased from 250ms to 2000ms
  const shortIntervalPeriod = 50;  // Reduced from 100 to 50
  ...
  async function _doPoll() {
    ...
    try {
      ...
      if (result) {
        shortIntervalCount = shortIntervalPeriod; // Reset the counter when the result is true
      ...
      if (errorCount <= maxErrorCount) {
        nextInterval = Math.min(longInterval * Math.pow(2, errorCount), 30000); // Max 30 seconds
      } else {
        nextInterval = 30000; // Cap at 30 seconds for persistent errors
      }
    }
  ...
  // Add initial delay to allow page to load first
  setTimeout(_doPoll, 1000);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    if (document.body.getAttribute('x-data')) {
      const bodyData = Alpine.$data(document.body);
      if (bodyData) bodyData.isLoading = false;
    }
  }, 2000); // Hide after 2 seconds to allow initial polling
});

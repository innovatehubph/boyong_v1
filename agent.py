...
# Import multimodal capabilities
try:
    from python.helpers.multimodal import process_multimedia_request, initialize_multimodal, get_multimodal_capabilities
    MULTIMODAL_AVAILABLE = True
    PrintStyle(font_color="cyan", padding=False).print("🎨 Multimodal AI capabilities loaded")
except ImportError as e:
    MULTIMODAL_AVAILABLE = False
    PrintStyle(font_color="yellow", padding=False).print(f"⚠️ Multimodal AI not available: {e}")
...
class AgentConfig:
    ...
    code_exec_ssh_enabled: bool = False  # Pareng Boyong Enhancement: Use local execution to avoid SSH socket issues
...
class Agent:
    ...
    def __init__(
        ...
        # non-config vars
        self.number = number
        self.agent_name = "Pareng Boyong" if self.number == 0 else f"A{self.number}"
